...
- 2026-07-06：修复考研数学题库 LaTeX 渲染不完全（`fixNonStandardMath`：`[matrix]`/`⎩⎨⎧` → 标准 LaTeX 环境，7 题套 86 处修复）；项目更名为「错题助手」；本地题库题套新增 PDF 导出（/api/quiz/export-set-pdf），带解析文件名追加「（解析）」；修正原 /api/quiz/export-pdf 单选答案字母转换
